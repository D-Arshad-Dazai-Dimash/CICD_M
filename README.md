...
### Создание Target Group для Application Load Balancer (ALB)

**Target Group** в AWS определяет, куда Application Load Balancer (ALB) будет направлять трафик. Вот пошаговая инструкция:

---

### 1. Перейдите в AWS Management Console
1. Зайдите в раздел **EC2**.
2. В левой панели выберите **Target Groups**.
3. Нажмите кнопку **Create target group**.

---

### 2. Выберите Тип Target Group
1. **Choose a target type**:
   - **Instances**: Если хотите направлять трафик на EC2 инстансы (самый распространенный случай).
   - **IP addresses**: Если хотите направлять трафик на IP-адреса (например, контейнеры вне AWS).
   - **Lambda function**: Если хотите направлять трафик на Lambda-функции.

Выберите **Instances** для работы с EC2.

---

### 3. Настройте основные параметры
1. **Target group name**:
   - Укажите имя для вашей Target Group (например, `my-target-group`).
2. **Protocol**:
   - Выберите `HTTP` (или `HTTPS`, если ваш ALB настроен на шифрование).
3. **Port**:
   - Укажите порт, на который будет направляться трафик (например, `5000`, если ваше приложение слушает на этом порту).
4. **VPC**:
   - Выберите VPC, в которой находятся ваши EC2 инстансы.

---

### 4. Настройте Health Checks (Проверка состояния)
1. **Health check protocol**:
   - Выберите `HTTP` (или `HTTPS`, если используете зашифрованный трафик).
2. **Health check path**:
   - Укажите путь для проверки состояния приложения, например:
     ```
     /
     ```
     или:
     ...
     /health
     ```
3. **Advanced health check settings** (необязательно):
   - **Healthy threshold**: Укажите минимальное количество успешных проверок для подтверждения, что инстанс "здоров".
     - Рекомендуется: `2`.
   - **Unhealthy threshold**: Укажите минимальное количество неудачных проверок для признания инстанса "нездоровым".
     - Рекомендуется: `2`.
   - **Timeout**: Максимальное время ожидания ответа от инстанса (в секундах).
     - Рекомендуется: `5`.
   - **Interval**: Интервал между проверками (в секундах).
     - Рекомендуется: `30`.
   - **Success codes**: Укажите ожидаемые HTTP коды для успешной проверки.
     - Например: `200` или `200,301`.

---

### 5. Зарегистрируйте EC2 инстансы
1. **Register targets**:
   - На шаге "Register targets" выберите ваши EC2 инстансы, которые будут получать трафик.
   - Укажите порт (например, `5000`), если он не задан автоматически.
2. Нажмите **Include as pending below**, чтобы добавить выбранные инстансы.

---

### 6. Завершите создание
1. Нажмите **Create target group**.
2. После создания Target Group, она станет доступной для использования с ALB.

---

### 7. Свяжите Target Group с ALB
1. Перейдите в **Load Balancers**.
2. Выберите ваш ALB.
3. Перейдите в раздел **Listeners** → **View/edit rules**.
4. Добавьте правило для направления трафика:
   - **If**: Укажите условия (например, все запросы).
   - **Then**: Выберите Target Group, созданную ранее.
